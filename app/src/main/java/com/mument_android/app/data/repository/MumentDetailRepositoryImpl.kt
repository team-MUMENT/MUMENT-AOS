...
class MumentDetailRepositoryImpl @Inject constructor(
    ...
    override suspend fun fetchMumentDetail(mumentId: Int, userId: Int): Flow<ApiResult<MumentDetailEntity>> = flow {
        emit(ApiResult.Loading<MumentDetailEntity>(null))
        val response = mumentDetailDataSource.fetchMumentDetail(mumentId, userId)
